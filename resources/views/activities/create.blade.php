            ...
            <div class="lg:flex lg:items-center lg:justify-between">
                <div class="flex-1 min-w-0">
                    <h2 class="text-2xl font-bold leading-7 text-gray-900 sm:text-3xl sm:truncate">
                        Create New Activity {{ $user ? " for {$user->name}" : "" }}
                    </h2>
                </div>
            </div>
        </div>
    ...
    <div class="py-12">
        <div class="mx-auto space-y-6 max-w-7xl sm:px-6 lg:px-8">
            <div class="p-4 bg-white shadow sm:p-8 sm:rounded-lg">
                <div class="max-w-xl">
                    <form method="post" action="{{ route('activities.store') }}" enctype="multipart/form-data" class="mt-6 space-y-6">
                        @csrf
                        <div>
                        ...
                        <div>
                            ...
                            <x-text-input id="desc" name="description" type="text" class="block w-full mt-1" :value="old('description')" required autocomplete="description" />
                            <x-input-error class="mt-2" :messages="$errors->get('description')" />
                        </div>

                        <div>
                            <x-input-label for="date" value="Activity Date" />
                            <x-text-input id="date" name="activity_date" type="date" class="block w-full mt-1" :value="old('activity_date') ?: $date" required autocomplete="activity_date" />
